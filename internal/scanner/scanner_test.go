...
func TestAnnotateAndFilter_Regex(t *testing.T) {
	now := time.Date(2026, 1, 17, 0, 0, 0, 0, time.UTC)
	old := time.Date(2025, 1, 1, 0, 0, 0, 0, time.UTC).Format(time.RFC3339)

	mods := []Module{
		{Path: "github.com/foo/bar", Version: "v1.0.0", Update: &Module{Version: "v1.2.0", Time: old}},
		{Path: "example.com/other", Version: "v1.0.0", Update: &Module{Version: "v1.1.0", Time: old}},
	}

	pattern := regexp.MustCompile(`^github\.com/foo`)
	out := AnnotateAndFilter(mods, nil, Options{Filter: `^github\.com/foo`, FilterRegex: pattern, IncludeAll: true}, now)
	if len(out) != 1 || out[0].Path != "github.com/foo/bar" {
		t.Fatalf("expected regex to match github module, got %#v", out)
	}
}
...
func TestGetUpdatesFrom_InvalidFilterRegex(t *testing.T) {
	orig := goListAllModulesOutput
	defer func() { goListAllModulesOutput = orig }()
...
`
	...
	if _, err := GetUpdatesFrom(goModPath, Options{Filter: "["}); err == nil {
